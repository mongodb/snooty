    ...
    const innerContent = (
      <section style={ {height:'100%'} }>
        <div className="guide__title"> 
          { this.props.card.name === 'card' ? this.props.card.argument[0].value : this.props.card.argument[0].value } 
        </div>
        <ul className="guide__body">
          { this.props.card.name === 'card' ? '' :
              this.props.card.children[0].children.map((listItem, index) => {
                return <li className="guide__entry" key={ index }><a href={ listItem.children[0].children[0].value }> { listItem.children[0].children[0].value } </a></li>
              })
          }
        </ul>
        <ul className="guide__pills"></ul>
        <div className="guide__time"> 5min </div>
      </section>
    );
