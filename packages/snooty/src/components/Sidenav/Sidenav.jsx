...
const getTopAndHeight = (topValue) => {
  return css`
    top: max(
      min(calc(${topValue} - var(--scroll-y))),
  ...
  ({ topLarge, topMedium, topSmall }) => css`
    ...
    height: calc(
      100vh + ${theme.header.actionBarMobileHeight} - ${topLarge} +
        min(
          calc(${topLarge} - ${theme.header.actionBarMobileHeight}),
          var(--scroll-y)
        )
    );
...
const additionalLinks = [
  {
    ...
    url: "https://support.mongodb.com/welcome",
  },
  {
    glyph: "Person",
...
const Sidenav = ({
  ...
  const { hideMobile, isCollapsed, setCollapsed, setHideMobile } =
  ...
  const isGuidesLanding =
    project === "guides" && template === "product-landing";
        ...
        <SidenavMobileTransition hideMobile={hideMobile} isMobile={isMobile}>
          ...
          >
            ...
            {!ia && !showAllProducts && (
              <>
                {isGuidesTemplate && (
                  <StyledChapterNumberLabel number={guidesChapterNumber} />
                )}
                <SideNavItem
                  as={Box}
