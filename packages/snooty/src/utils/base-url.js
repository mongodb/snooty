...
export const productToPrefixMapping = (product) => {
  const mapping = {
...
export const decomposeProductPrefixAndPath = (url) => {
  const decomposedUrl = url.toString().split(".");
  const subdomainProduct =
    decomposedUrl.length > 2 && decomposedUrl[1] !== "mongodb"
      ? decomposedUrl[1]
      : "";
...
export const baseUrl = (url = DOTCOM_BASE_URL, options = {}) => {
  ...
  const { productPrefix, pathname } = decomposeProductPrefixAndPath(
    new URL(url)
  );

  // Check that our pathname doesn't already include the base prefix.
  const hasDuplicatePrefix = pathname
    .replace("/", "")
    .startsWith(DOTCOM_BASE_PREFIX);
  let baseUrl = new URL(
    needsPrefix && !hasDuplicatePrefix
      ? joinUrlAndPath(DOTCOM_BASE_URL, DOTCOM_BASE_PREFIX)
      : `${DOTCOM_BASE_URL}`
  );
  ...
  if (pathname.length >= 1)
    baseUrl.pathname = joinUrlAndPath(baseUrl.pathname, pathname);

  return assertTrailingSlash(
    needsProtocol ? baseUrl.toString() : baseUrl.toString().split("//")[1]
  );
};
