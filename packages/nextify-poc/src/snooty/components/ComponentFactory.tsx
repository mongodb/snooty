...
const getComponent = (() => {
  let componentMap:
    | Record<Exclude<ComponentType, "block-quote">, React.ComponentType<any>>
    | undefined = undefined;
  return (key: keyof typeof componentMap | string) => {
...
function getComponentType(
  ...
  // @ts-ignore
  let ComponentType: React.ComponentType<any> | undefined = lookup
    ? getComponent(lookup)
    : undefined;
...
const ComponentFactory = (props: ComponentFactoryProps) => {
  ...
  const selectComponent = () => {
    ...
    if (!ComponentType) {
      console.warn(
        `${type} ${name ? `"${name}" ` : ""}not yet implemented${
          slug ? ` on page ${slug}` : ""
        }`
      );
      return null;
