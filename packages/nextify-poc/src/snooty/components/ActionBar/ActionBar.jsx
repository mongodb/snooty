...
const ActionBar = ({ template, slug, sidenav, ...props }) => {
  ...
  const feedbackData = useFeedbackData({
    ...
    title:
      getPlaintext(
        getNestedValue(["slugToTitle", slug === "/" ? "index" : slug], metadata)
      ) || "MongoDB Documentation",
  });

  const { fakeColumns, containerClassname, searchContainerClassname } =
    getContainerStyling(template);
  ...
  const CHATBOT_SERVER_BASE_URL = ["dotcomprd", "production"].includes(
    snootyEnv
  )
  ...
  return (
    <div
      className={cx(
        ...
        actionBarStyling,
        containerClassname,
        isOfflineDocsBuild ? offlineStyling : ""
      )}
    >
      {fakeColumns && <div></div>}
      <ActionBarSearchContainer className={cx(searchContainerClassname)}>
        {sidenav && (
          <Overline
            className={cx(overlineStyling)}
            onClick={() => setHideMobile((state) => !state)}
          >
      ...
      {!isOfflineDocsBuild && (
        <ActionsBox>
          ...
          <IconButton
            className={chatbotMobileButtonStyling}
            aria-label={CHATBOT_TEXT}
            onClick={openChatbot}
          >
            <Icon glyph={"Sparkle"} />
