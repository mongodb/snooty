...
describe('baseUrl', () => {
  it('by default returns the DOTCOM_BASE_URL with protocols and prefix', () => {
...
// Test cases from search-specific cases.
it('rewrites old search urls to new ones', async () => {
  const mapping = [
    [
  ...
  // Old to new, and new to new
  for (const pair of mapping) {
    expect(baseUrl(pair[0])).toStrictEqual(pair[1]);
    expect(baseUrl(pair[1])).toStrictEqual(pair[1]);
  }
});
